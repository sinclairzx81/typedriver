...
const Test = Assert.Context('Validator.JsonSchema.TypeBox')
...
Test('Should Check 4', () => {
  const T = compile(t.Array(t.Number()))
...
Test('Should Check 5', () => {
  const T = compile(t.Tuple([t.Number(), t.Boolean()]))
  type T = Static<typeof T>
  Assert.IsExtendsMutual<T, [number, boolean]>(true)
  Behaviors(T, [[1, true]], [[true, true], null])
})
Test('Should Check 6', () => {
  const T = compile(t.Union([t.Number(), t.String()]))
  type T = Static<typeof T>
